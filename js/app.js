...
const state = {
  view: "home", // home | main | sector | result
  mainIndex: 0,
  sectorIndex: 0,
  mainAnswers: {},
  sectorCounts: {},
...
function renderHome() {
  const grid = Object.entries(TYPES)
    .map(([code, t]) => `
      <div class="type-tile">
        <span class="type-tile__emoji">${t.emoji}</span>
        <span class="type-tile__code mono">${code}</span>
      </div>`)
    .join("");
  ...
  root.innerHTML = `
    <div class="screen screen--home">
      ...
      <div class="home-types">
        <p class="mono accent center label">16 TYPES OF JAPANESE STOCK INVESTORS</p>
...
function renderMain() {
  ...
  const options = SCALE.map(
    (s) => `<button class="scale-btn" data-a="${s.a}">${s.label}</button>`
  ).join("");
  ...
  root.querySelectorAll(".scale-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      state.mainAnswers[q.id] = Number(btn.dataset.a);
...
function renderResult() {
  ...
  root.innerHTML = `
    <div class="screen result">
      ...
      <p class="lead center result-desc">${esc(type.desc)}</p>
